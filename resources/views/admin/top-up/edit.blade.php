    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card card-body">
                <h3 class="box-title m-b-0">Edit Top Up</h3>
                <br>
                <div class="row">
                    <div class="col-sm-10 col-xs-12">
                        <form method="post" action="{{route('addon.update',$topup->id)}}" enctype="multipart/form-data">
                        {{ method_field('put') }}
                            {{csrf_field()}}
                            <div class="form-group">
                                <label for="exampleInputEmail1">Top Up Name</label>
                                <input type="text" name="name" value="{{$topup->name}}" class="form-control" placeholder="Enter Name"
                                    required>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Price</label>
                                <input type="text" name="price" value="{{$topup->price}}" class="form-control" onkeypress="return event.charCode >= 48 && event.charCode <= 57" placeholder="Enter Price"
                                    required>
                            </div> 
                            <img src="{{asset('public/assets/topup-images/'.$topup->image)}}" width="50"
                                height="50">
                            <div class="form-group">
                                <label for="exampleInputPassword1">Image</label>
                                <input type="file" class="form-control" name="topup_image" placeholder="" >
                            </div>
                        
                            <div class="form-group">
                                <label for="exampleInputPassword1"> Description</label>
                                <textarea class="form-control" name="description">{{$topup->description}}</textarea>
                            </div>
                         
                            <div class="form-group">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" name="enabled" class="custom-control-input"
                                        id="customSwitch1" {{$topup->enabled == 1 ? 'checked': ''}}>
