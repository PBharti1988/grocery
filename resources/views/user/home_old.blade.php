...
@foreach($val->$category as $val1)
    <div id="pop" class="{{str_replace(' ', '_',$val1->item_name)}} close{{str_replace(' ', '_',$val1->item_name)}}  ">
      
      
            <div id="allpopupContact">
                             
                <div class="pop-container">
                    <?php $img =$val1->item_name; ?>
                   
                    <div class="slider-section">
                          <div class="owl-carousel owl-theme">                     
                              @if(!empty($val1->$img))
                            @foreach($val1->$img as $val2)
                            <div class="item">
                            ...
                            @if($val1->image !="")
                             ...
                             <img src="{{asset('public/assets/images/item-images/'.$val1->image)}}">
                               </div>
                                 @endif 
                            @endif
                          </div>
                    ...
                    <div class="product-detail text-black">
                        <h1>{{$val1->item_name}}</h1>
                        <span><i class="fa fa-inr">{{$val1->discount_price}}</i></span>
                        <span class="item_discount"><del><i class="fa fa-inr"><del>{{$val1->item_price}}</i></del></span>
                    </div>
                    <ul class="product-content">
                        <li>
                            <h1>Description</h1>
                            <p>{{$val1->long_description}}</p>
                        </li>
                        <?php $desc ='desc'.$val1->item_name; ?>
                          @if(!empty($val1->$desc))
                      @foreach($val1->$desc as $val3)
                        <li>
                            <h1>{{$val3->title}}</h1>
                ...
                <div id="close">
                    <button class="close" data-class="close{{str_replace(' ', '_',$val1->item_name)}}" onclick ="div_hide()">Close</button>
                </div>
            </div>
...
-->
            ...
            @if($count%2==1)
            <article class="{{$articleClass}} {{str_replace(' ','_',$val->category_name)}}">
            @endif

                  <div class="@if($j%2==1){{'first-box'}} @else{{'second-box'}}@endif box-category {{$val1->item_name}}" data-class="{{str_replace(' ', '_',$val1->item_name)}}" style="background:{{$val1->card_color}}" onclick="div_show()">
                    <div class="pro-type">
                        <div class="@if($val1->item_type == 'veg') veg-product @else non-veg-product @endif"></div>
                    </div>
                    @if($val1->image !="")
                    <div class="product-img">
                        <img src="{{asset('public/assets/images/item-images/'.$val1->image)}}">
                    ...
                    <div class="product-detail text-white">
                        <h1>{{$val1->item_name}}</h1>
                        <span><i class="fa fa-inr"></i>{{$val1->discount_price}}</span><span style="margin-left:7px;"><del><i class="fa fa-inr"></i>{{$val1->item_price}}</del></span>
                        <p>{{$val1->short_description}}</p>
                    </div>
                </div>
                <?php $j++;   ?>
            @if($count%2==0)
...
$( function() {
  ...
  $('.sub').click(function(){
   ...
   // $('.sub-cat').fadeIn().next().delay(500).fadeOut(); 
  });
